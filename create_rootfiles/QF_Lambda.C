...
void QF_Lambda(){
  ...
  //TFile * fout = new TFile("train_QF_Lambda.root","RECREATE");
  //TFile * fout = new TFile("test_QF_Lambda.root","RECREATE");
  TFile * fout = new TFile("testHigh_QF_Lambda.root","RECREATE");
 ...
 double masses2[2] = {M_Proton,M_Pi};

 TTree * tree = new TTree("tree","tree of data");
 //tree -> Branch("P_K_Lab",&P_K_Lab,"P_K_Lab/D"); 
 // tree->Branch("p_Pi","TLorentzVector",&pPi_mom,32000,1);
 // tree->Branch("p_Y","TLorentzVector",&pY_mom,32000,1);
 // tree->Branch("p_Spec","TLorentzVector",&pSpec_mom,32000,1);
 // tree->Branch("DecayP","TLorentzVector",&DecayP_mom,32000,1);
 // tree->Branch("DecayPi","TLorentzVector",&DecayPi_mom,32000,1);
 // tree->Branch("mm_p",&mm_p, "mm_p/D");
...
//---------------pi-n_reaction--Sigma_production---------------------------
   ...
   DecayP_mom = event2.GetDecay(0);
   DecayPi_mom = event2.GetDecay(1);
